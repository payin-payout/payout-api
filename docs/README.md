...
*В случае получения ошибки от сервера, вместо ожидаемого ответа на запрос - не является финализирующим статусом и в таком случае необходимо обратиться с запросом к getPaymentStatus для уточнения статуса платежа.*
...
```
*В поле uid допускается указание id который был ранее указан при проведении платежа*
